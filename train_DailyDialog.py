...
from model import MaskedNLLLoss, LSTMModel, GRUModel, DialogueGCNModel, DialogueGCN_DailyModel
...
def train_or_eval_graph_model(model, loss_function, dataloader, epoch, cuda, optimizer=None, train=False):
    ...
    for i, data in enumerate(dataloader):
        ...
        textf, qmask, umask, label = process_data_loader(data)
        # textf, qmask, umask, label = [d.cuda() for d in data[:-1]] if cuda else data[:-1]
        ...
        if train:
            ...
            optimizer.step()
        if train and i % 10 == 0:
            avg_loss_p = round(np.sum(losses) / len(losses), 4)
            preds_ = np.concatenate(preds)
            labels_ = np.concatenate(labels)
            labels_ = np.array(labels_)
            preds_ = np.array(preds_)

            avg_accuracy_p = round(accuracy_score(labels_, preds_) * 100, 2)
            avg_fscore_p = round(f1_score(labels_, preds_, average='micro', labels=[0, 1, 2, 4, 5, 6]) * 100, 2)
            # Add precision and recall
            precision_p = round(precision_score(labels_, preds_, average='micro', labels=[0, 1, 2, 4, 5, 6]) * 100, 2)
            recall_p = round(recall_score(labels_, preds_, average='micro', labels=[0, 1, 2, 4, 5, 6]) * 100, 2)
            print(
                'steps {} train_loss {} train_acc {} train_fscore {} train_precision: {}, train_recall: {} time {}s'.format(
                    i, avg_loss_p, avg_accuracy_p, avg_fscore_p, precision_p, recall_p, round(time.time() - start_time, 2)))
            # print(
            #     'steps {} train_loss {} train_acc {} train_fscore {} test_loss {} test_acc {} test_fscore {} time {}s'.format(
            #         i, avg_loss_p, avg_accuracy_p, avg_fscore_p, test_loss, test_acc,
    ...
    # Add precision and recall
    precision = round(precision_score(labels, preds, average='micro') * 100, 2)
    recall = round(recall_score(labels, preds, average='micro') * 100, 2)
